...
test('does it add to the accounts', () => {
    ...
    expect(controller.addAccount("checking", 100)).toEqual([{ accountName: 'checking', balance: 100, key: 1 }]);
    expect(controller.addAccount("savings", 1000)).toEqual([{ accountName: 'checking', balance: 100, key: 1 }, { accountName: 'savings', balance: 1000, key:2 }]);
});
...
test('does it remove accounts from the array', () => {
    ...
    expect(controller.accountArray).toEqual([{ accountName: 'savings', balance: 1000, key:1 }]);
...
test('which account has the most', () => {
    ...
    controller.addAccount("checking", 100);
    controller.addAccount("savings", 1000);
    expect(controller.biggestAccount()).toEqual(1000);
    controller.addAccount("credit", 6000);
    expect(controller.biggestAccount()).toEqual(6000);
});
...
test('which account has the least', () => {
    ...
    controller.addAccount("savings", 1000);
    controller.addAccount("credit", 6000);
    expect(controller.smallestAccount()).toEqual(100);
    controller.removeAccount("checking");
    controller.removeAccount("savings");
    expect(controller.smallestAccount()).toEqual(6000);
});